...
def make_trend_charts(room_w, room_d, room_h, ach, n_st, n_si, n_ly, current_t, panic_mode, initial_blackout_t, depth_m, grid_destroyed, ext_cfg, filter_on):
    ...
    for t in times:
        _, c, _, temp, _, _, _, tox, rad = compute_transient_baseline(room_w, room_d, room_h, ach, n_st, n_si, n_ly, t, panic_mode, initial_blackout_t, depth_m, grid_destroyed, ext_cfg, filter_on)
...
def make_heatmap_fig(room_w, room_d, X, Y, Z, eqs, t, n_st, n_si, n_ly, title, scale, zmin, zmax, cbtitle):
    ...
    if n_st > 0: fig.add_trace(go.Scatter(x=st_x, y=st_y, mode="markers", name="Standing", marker=dict(size=6, color="black")))
